...
import {inputData} from '../src/pageObjects/inputData'
...
test('AngularJS WebTable', async({page}) =>{
  ...
  const InputData = new inputData(page);
  await InputData.goto();
  await expect(page).toHaveTitle('Protractor practice website - Banking App'); //For page title

  await InputData.bankManagerLogin()

  await InputData.customersLink()
  await page.waitForTimeout(2000)
  ...
  // await page.locator('placeholder="First Name"').pressSequentially('TestUser FirstName',{ delay: 5 })
  // expect(page.locator('placeholder="First Name"')).toHaveValue('TestUser FirstName')

  // await page.locator('placeholder="Last Name"').pressSequentially('TestUser LastName',{ delay: 5 })
  // expect(page.locator('placeholder="Last Name"')).toHaveValue('TestUser LastName')

  // await page.locator('placeholder="Post Code"').pressSequentially('123456',{ delay: 5 })
  // expect(page.locator('placeholder="Post Code"')).toHaveValue('123456')
